...
def _parse_func(source: str) -> ast.FunctionDef:
    """Вспомогательная функция: парсим исходник и возвращаем первый FunctionDef."""
...
class TestIsEmptyMethod:
    ...
    def test_pass_is_empty(self):
        func = _parse_func("""
            def foo(self):
                pass
        """)
        assert CohesionAdapter._is_empty_method(func) is True

    # тело из одного Ellipsis — типичная заглушка в Protocol/ABC
    def test_ellipsis_is_empty(self):
        func = _parse_func("""
            def foo(self):
                ...
        """)
    ...
    def test_raise_nie_call_is_empty(self):
        func = _parse_func("""
            def foo(self):
    ...
    def test_raise_nie_name_is_empty(self):
        func = _parse_func("""
            def foo(self):
                raise NotImplementedError
        """)
    ...
    def test_raise_nie_with_message_is_empty(self):
        func = _parse_func("""
            def foo(self):
    ...
    def test_docstring_and_pass_is_empty(self):
        func = _parse_func("""
            def foo(self):
                \"\"\"Документация.\"\"\"
                pass
    ...
    def test_assignment_not_empty(self):
        func = _parse_func("""
            def foo(self):
                self.x = 1
        """)
        assert CohesionAdapter._is_empty_method(func) is False

    # вызов функции — реальная логика
    def test_call_not_empty(self):
        func = _parse_func("""
            def foo(self):
                self.bar()
        """)
    ...
    def test_raise_other_not_empty(self):
        func = _parse_func("""
            def foo(self):
    ...
    def test_async_pass_is_empty(self):
        func = _parse_func("""
            async def foo(self):
                pass
        """)
